...
knitr::include_graphics("images/ebusiness.png")
```
...
```{block2, st:onlinepayment, type='rmdextra'}
...
```{block2, ex:jobdescriptions, type='rmdexercise'}
...
```{block2, ex:OrangeHRM, type='rmdexercise'}
...
```{block2, ex:EmployeeChurn, type='rmdexercise'}
...
* **Support:** fraction of all transactions that contain both $X$ and $Y$. This is a measure of how often a pair of items occur.

$$Support(X,Y) = \frac{t_{(X,Y)}}{t_{(all)}}$$

* **Confidence:** fraction of transactions that contain both $X$ and $Y$ divided by the number that only contain $X$. This a measure of how often the two items occur together.

$$Confidence(X,Y) = \frac{Support(X,Y)}{Support(X)} = \frac{t_{(X,Y)}}{t_{(X)}}$$
* **Lift:** This is a measure of how the confidence compares to the expected confidence, (i.e, how much confidence that $X$ will be purchased given that $Y$ was purchased). 
